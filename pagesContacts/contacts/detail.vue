  ...
  <view class="container">
    <view v-if="data">
      <uni-list :border="false" class="uhead">
        <uni-list-item
          :thumb="data.avatar ? data.avatar : '/static/txl/ico_logo_@3x.png'"
          thumb-size="lg"
        >
          <view slot="body">
            <view class="uni-list-item__content">
              <text class="re-ufont">{{ data.nickname }}</text>
              <text class="re-uni-list-item__content-note">{{
                data.post
              }}</text>
            </view>
          </view>
      ...
      <uni-list>
        <uni-list-item>
          <template slot="header">
            <image
              class="slot-image"
              src="/static/txl/ico_page_@3x.png"
              mode="widthFix"
            ></image>
          </template>
          <view slot="body">
        ...
        <uni-list-item>
          <view slot="body">
            <view class="re-uni-list-item__content">
              ...
              <text class="re-uni-list-item__content-title">{{
                data.nickname
              }}</text>
            </view>
          </view>
      ...
      <view class="message-button" v-if="id">
        <view class="button" @click="sendMessage()"> 发送消息 </view>
      </view>
    </view>
  </view>
</template>
...
export default {
  ...
  onLoad(e) {
    if (e.id !== uni.getStorageSync("userInfo")._id) this.id = e.id;
    this.userId = e.id;
    this.getUser(e.id);
    if (uni.getStorageSync("userInfo").role == "admin") this.ContactsEdit = true;
  },
  methods: {
    //发送消息
    async sendMessage() {
    ...
    getUser(id) {
      return db.collection("uni-id-users,opendb-department")
        .where({_id: id,})
        .field("department_id{name},username,nickname,post,mobile,email,avatar")
        .get()
        .then((res) => {
