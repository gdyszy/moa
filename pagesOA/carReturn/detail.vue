  ...
  <view style="width: 100%; background-color: #f5f5f7; padding-bottom: 150rpx">
    ...
    >
      <view class="baseMsg_detail">
        ...
        <view class="baseMsg_item">
          <view class="baseMsg_item_title">车辆借出时间</view>
          <view class="baseMsg_content">{{ listDetail.car_begin_time }}</view>
        </view>

        <view class="baseMsg_item">
          <view class="baseMsg_item_title">车辆退还时间</view>
          <view class="baseMsg_content">{{ listDetail.car_end_time }}</view>
        </view>
        ...
        <view class="baseMsg_item">
          <view class="baseMsg_item_title">车辆状态</view>
          {{ listDetail.explain }}
        </view>
        <view class="status_content"> </view>
          ...
          <view class="cardata">
            <view class="cardata-box">
              <view class="carnumber">
                {{ listDetail.carInfoRes.car_num?listDetail.carInfoRes.car_num:'' }}
              </view>
              <view class="carinfo">
        ...
        <view style="margin-bottom: 15px">
          ...
          <view class="carimg">
            <image
              v-for="(item, index) in listDetail.attachments"
              :key="index"
              :src="item"
              mode=""
    ...
    <view v-if="result == 30 && type">
      <btn @click="adpot(0)" text="审核"></btn>
    </view>

    <!-- <view v-if="listDetail.result !== 10 && !type">
      <view class="bottom">
        ...
        <view class="bottom_btn" @tap="openPopup">
          ...
          <view class="bottom_text">撤回</view>
        </view>
      </view>
    </view> -->
...
export default {
  ...
  methods: {
    clickImg(index) {
      wx.previewImage({
        urls: this.listDetail.attachments,
        current: index,
      });
    },
    userCarTime() {
    ...
    revise() {
      uni.navigateTo({
        url: "/pagesOA/carReturn/index?info=" + JSON.stringify(this.listDetail),
      });
    },
    ...
    getDailyInfo(id, gwflowid) {
      ...
      uniCloud
        .callFunction({
          name: "gongwen",
          data: {
            name: this.type ? "gwGetDBDetail" : "gwGetDetail",
            data: formData,
          },
        })
        .then((res) => {
