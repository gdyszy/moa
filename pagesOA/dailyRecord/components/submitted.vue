...
export default {
  ...
  methods: {
        ...
        .then((res) => {
          if (res.result == "无权限访问。") {
            ...
            for (let i in this.dailySubmitList) {
              this.dailySubmitList[i].dailyList[0].create_date = timeFormat(
                this.dailySubmitList[i].dailyList[0].create_date,
                "hh:mm"
              );
              let flag = false;
              for (let j in this.dailySubmitList[i].dailyList[0].readStatus) {
                if (
                  this.dailySubmitList[i].dailyList[0].readStatus[j].user_id ==
                  this.username
                ) {
                  flag = true;
                }
    ...
    routeTo(id, uid, state) {
      uni.navigateTo({
        url:
          "/pagesOA/daily/detail?id=" + id + "&uid=" + uid + "&state=" + state,
      });
    },
  },
};
